...
class task_constructor_rosenbrock(object):
    ...
    def __call__(self, x_vals, iteration_directory, **extra_args):
        ...
        for var, val, para_file, para_file_format in zip(x_vars, x_vals, para_files, para_file_formats):
            ...
            update_parameter_in_file(os.path.join(path_to_stage_base_dir, para_file), var, val, para_file_format)
...
class RosenbrockScript(SessionBasedScript):
    ...
    def __init__(self):
        SessionBasedScript.__init__(self, version='0.2', stats_only_for=Application)
    ...
    def new_tasks(self, extra):
        ...
        initial_pop = draw_population(
            lower_bds=lower_bounds, upper_bds=upper_bounds, size=self.params.pop_size, dim=dim
        )
        ...
        kwargs['task_constructor'] = task_constructor_rosenbrock(
            path_to_exectuable=self.params.path_to_executable, path_to_base_dir=self.params.path_to_base_dir
        )
    ...
    def setup_options(self):
        self.add_param("--pop_size", metavar="ARCH", type=int, dest="pop_size", default='100', help="Population size. ")
        self.add_param(
            "--y_conv_crit",
            metavar="ARCH",
            type=float,
        ...
        self.add_param(
            ...
            metavar="ARCH",
            type=str,
            dest="path_to_executable",
            default=os.path.join(os.getcwd(), 'bin/rosenbrock'),
            help="Directory in which to perform optimization",
        )
        self.add_param(
            "--path_to_base_dir",
            metavar="ARCH",
            type=str,
            dest="path_to_base_dir",
            default=os.path.join(os.getcwd(), 'base'),
            help="Directory in which to perform optimization",
        )
