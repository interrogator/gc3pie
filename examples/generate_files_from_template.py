...
def match_ispher_with_basis(kw):
    ...
    if kw.has_key("SIMPLEBASIS") and val("SIMPLEBASIS") in ["CCD", "CCT", "CCQ", "CC5", "CC6"]:
...
def acceptable_gbasis_and_ngauss(kw):
    ...
    if (val("NFFUNC") == "NFFUNC=1" or val("NDFUNC") == "NDFUNC=1") and (
        val("NGAUSS") not in ["5", "6"] or val("GBASIS") not in ["N31", "N311"]
    ):
        return False
...
H     1.0   0.6252197764   0.6252197764   0.6252197764""",
    ...
    SCF=[
        Template("SCFTYP=${SCFTYP}", SCFTYP=["RHF", "ROHF", "UHF"]),  # "MPLEVL=${MPLEVL}", # NODFT
    ...
    BASIS=[
        Template(
            "GBASIS=${SIMPLEBASIS}", SIMPLEBASIS=["MINI", "MIDI", "DZV", "TZV", "CCD", "CCT", "CCQ", "CC5", "CC6"]
        ),
        Template(
