...
class TopBlastScript(SessionBasedScript):
    ...
    def setup_args(self):
        self.add_param("query", help="Query file (FASTA format)", type=existing_file)
        self.add_param(
            "dir",
            nargs="+",
            type=existing_directory,
            help=("Directories to scan for `.faa` files" " to compare to the query."),
        )
            ...
            for rootpath, dirs, files in os.walk(dir):
                for filename in files:
                    if filename.endswith(".faa"):
                        ...
                        apps_to_run.append(
                            BlastApp(self.params.query, vs_file, self.params.e_value, self.params.output_fmt)
                        )
        return apps_to_run
