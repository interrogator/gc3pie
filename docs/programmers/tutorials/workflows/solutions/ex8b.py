...
class BlastDbScript(SessionBasedScript):
    ...
    def setup_args(self):
        ...
        self.add_param("known_faa", nargs="+", help="Samples to compare to the query")

    def setup_options(self):
        self.add_param("--e-value", "-e", type=float, help="Expectation value")
        self.add_param("--output-format", "-o", dest="output_fmt", type=int, help="Output format, int from 0 to 9")

    def new_tasks(self, extra):
        return [BlastDbTasks(self.params.new_faa, self.params.known_faa, self.params.e_value, self.params.output_fmt)]
...
class BlastpgpApp(Application):
    ...
    def __init__(self, query_file_path, db_files_path, e_value, output_fmt):
        ...
        Application.__init__(
            self,
            arguments=["blastpgp", "-i", q, "-d", db, "-e", e_value, "-m", output_fmt, "-o", "output.txt"],
            inputs=([q] + db_files),
            outputs=["output.txt"],
            # todo: fix inp1 and inp2 below ... they were undefined
