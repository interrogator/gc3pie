...
class _TestsCommon(object):
    ...
    def setUp(self):
        ...
        self.basedir = tempfile.mkdtemp(prefix=(__name__ + '.'), suffix='.tmp.d')
        orig_wd = os.getcwd()
...
resourcedir = {basedir}/resource.d
    ...
    def run(self, cmd):
        ...
        rc = proc.returncode
        gc3libs.log.debug("Log of command `%s`:\n```\n%s\n```", ' '.join(cmd), stdout)
        assert rc == 0, "Command `{0}` exited with non-zero exitcode {1}".format(' '.join(cmd), rc)
        return (rc, stdout)
    ...
    def check_with_timeout(self, condition, timeout, errmsg):
        ...
        for _ in range(timeout):
            ...
            assert False, (errmsg + " (within {timeout} seconds)").format(timeout=timeout)
...
class TestSessionBasedScript(_TestsCommon):
    ...
    def test_session_based_script(self):
        ...
        rc, stdout = self.run(
            [self.script_py, '-C', '1', '-s', 'session', '--config-files', self.cfgfile, '-r', 'localhost']
        )

        assert re.match(r'.*TERMINATED\s+3/3\s+\(100.0+%\).*', stdout, re.S)

        assert isdir(join(self.basedir, 'SimpleScript.out.d'))
        assert isfile(join(self.basedir, 'SimpleScript.out.d', 'SimpleScript.stdout'))

        assert isdir(join(self.basedir, 'SimpleScript.out2.d'))
        assert isfile(join(self.basedir, 'SimpleScript.out2.d', 'SimpleScript.stdout'))

        session_dir = join(self.basedir, 'session')
        assert isdir(session_dir)

        assert isfile(join(session_dir, gc3libs.session.Session.INDEX_FILENAME))

        assert isfile(join(session_dir, gc3libs.session.Session.STORE_URL_FILENAME))
...
@pytest.mark.skipif(sys.version_info < (2, 7), reason="python-daemon requires python 2.7+")
class TestSessionBasedDaemon(_TestsCommon):
    ...
    def daemon(self):
        ...
        self.check_with_timeout(condition=lambda: exists(pidfile), timeout=15, errmsg="PID file not created")
    ...
    def test_session_based_daemon_new_tasks(self, daemon, max_wait=10):
        ...
        self.check_with_timeout(
            condition=check_for_terminated, timeout=max_wait, errmsg="Daemon didn't complete initial task"
        )

        # check that output directory is there
        output_dir = join(self.basedir, 'EchoApp')
        self.check_with_timeout(
            condition=lambda: isdir(output_dir), timeout=max_wait, errmsg="Daemon didn't write output directory"
        )
    ...
    def test_session_based_daemon_inbox(self, daemon, max_wait=10):
        ...
        self.check_with_timeout(
            condition=lambda: isdir(inboxdir), timeout=max_wait, errmsg="Daemon didn't create inbox dir"
        )
        ...
        self.check_with_timeout(
            condition=lambda: isdir(output_dir), timeout=max_wait, errmsg="Daemon didn't process incoming file"
        )
    ...
    def test_session_based_daemon_command_quit(self, daemon, max_wait=60):
        ...
        self.check_with_timeout(
            condition=lambda: not exists(pidfile), timeout=max_wait, errmsg="Daemon didn't shut down"
        )
    ...
    def test_session_based_daemon_reload_session(self, max_wait=60):
        ...
        output_dir = join(self.basedir, 'EchoApp')
        self.check_with_timeout(
            condition=lambda: isdir(output_dir), timeout=max_wait, errmsg="Daemon didn't complete initial task"
        )
