...
def test_create():
    tmpdir = tempfile.mktemp(prefix=(os.path.basename(__file__) + '.'), suffix='.d')
    try:
        sess = Session(tmpdir)
...
class TestSession(object):
    ...
    def test_session_directory_created(self):
        ...
        assert os.path.samefile(os.path.join(os.path.abspath('.'), self.tmpdir), self.sess.path)
    ...
    def test_empty_lines_in_index_file(self):
        ...
        """
        self.sess.add(_PStruct(a=1, b='foo'), flush=True)
        with open(os.path.join(self.sess.path, self.sess.INDEX_FILENAME), 'a') as fp:
            fp.write('\n\n\n')
            fp.flush()
    ...
    def test_creation_of_timestamp_files(self):
        start_file = os.path.join(self.sess.path, self.sess.TIMESTAMP_FILES['start'])
        end_file = os.path.join(self.sess.path, self.sess.TIMESTAMP_FILES['end'])
    ...
    def test_standard_session_iterator_for_tasks(self):
        ...
        assert set(('task-1', 'task-2', 'task-3')) == set(job.jobname for job in self.sess)

    def test_standard_session_iterator_for_tasks_and_task_collections(self):
        coll = TaskCollection(jobname='collection', tasks=[Task() for i in range(3)])
        self.sess.add(coll)

        assert ['collection'] == [job.jobname for job in self.sess]

    def test_workflow_iterator_for_session(self):
        coll = TaskCollection(jobname='collection', tasks=[Task(jobname='task-%d' % i) for i in range(3)])
        coll2 = TaskCollection(jobname='collection-1', tasks=[Task(jobname='task-1-%d' % i) for i in range(3)])
        coll.tasks.append(coll2)

        self.sess.add(coll)

        assert ['collection', 'task-0', 'task-1', 'task-2', 'collection-1', 'task-1-0', 'task-1-1', 'task-1-2'] == [
...
class StubForSqlSession(TestSession):
    def test_sql_store(self):
        ...
        self.sess.flush()
        q = sql.select([self.sess.store.t_store.c.id]).where(self.sess.store.t_store.c.id == jobid)
...
class TestSqliteSession(StubForSqlSession):
    ...
    @pytest.fixture(autouse=True)
    def _with_tmp_session(self):
        self.tmpdir = os.path.abspath(tempfile.mktemp(prefix=(os.path.basename(__file__) + '.'), suffix='.d'))
        self.sess = Session(self.tmpdir, create=True, store_or_url="sqlite:///{0}/store.db".format(self.tmpdir))
        self.extra_args = {}
...
class TestMysqlSession(StubForSqlSession):
    ...
    @pytest.fixture(autouse=True)
    def _with_tmp_session(self):
        self.tmpdir = tempfile.mktemp(prefix=(os.path.basename(__file__) + '.'), suffix='.d')
        self.tablename = os.path.basename(tmpdir)
        self.extra_args = {'table_name': self.tablename}
        try:
            self.sess = Session(tmpdir, store_url="mysql://gc3user:gc3pwd@localhost/gc3", **self.extra_args)
