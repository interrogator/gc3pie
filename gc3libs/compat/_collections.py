...
try:
    ...
    # Backport of OrderedDict() class that runs on Python 2.4, 2.5,
    # 2.6, 2.7 and pypy.
    #
    # Passes Python2.7's test suite and incorporates all the latest
    # updates.
    ...
    class OrderedDict(dict):
        ...
        # An inherited dict maps keys to values.  The inherited dict
        # provides __getitem__, __len__, __contains__, and get.  The
        # remaining methods are order-aware.  Big-O running times for
        # all methods are the same as for regular dictionaries.

        # The internal self.__map dictionary maps keys to links in a
        # doubly linked list.  The circular doubly linked list starts
        # and ends with a sentinel element.  The sentinel element
        # never gets deleted (this simplifies the algorithm).  Each
        # link is stored as a list of length three: [PREV, NEXT, KEY].
        ...
        def __setitem__(self, key, value, dict_setitem=dict.__setitem__):
            ...
            # end of the linked list, and the inherited dictionary is
            # updated with the new key/value pair.
            if key not in self:
                root = self.__root
                last = root[0]
        ...
        def update(*args, **kwds):
            '''od.update(E, **F) -> None.
            Update od from dict/iterable E and F.
        ...
        def pop(self, key, default=__marker):
            ...
            corresponding value.  If key is not found, d is returned
