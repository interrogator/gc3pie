...
class FilesystemStore(Store):
    ...
    def __init__(
        self, directory=gc3libs.Default.JOBS_DIR, idfactory=IdFactory(), protocol=DEFAULT_PROTOCOL, **extra_args
    ):
        if isinstance(directory, Url):
            super(FilesystemStore, self).__init__(directory)
            directory = directory.path
        else:
            super(FilesystemStore, self).__init__(Url(scheme="file", path=os.path.abspath(directory)))
    ...
    def list(self):
        if not os.path.exists(self._directory):
            return []
        return [id_ for id_ in os.listdir(self._directory) if not id_.endswith(".OLD")]
    ...
    def load(self, id_):
        ...
        for source in sources:
            if not os.path.exists(source):
                gc3libs.log.debug("Cannot load object %s from '%s':" " file does not exist", id_, source)
                continue
            try:
                ...
                gc3libs.log.warning(
                    "Failed loading file '%s': %s: %s", filename, ex.__class__.__name__, ex, exc_info=True
                )
        else:
            # complain loudly
            raise gc3libs.exceptions.LoadError(
                "Failed loading object %s from file(s) %r."
                " (Earlier log lines may provide more details.)" % (id_, sources)
            )

        # minimal sanity check
        if not hasattr(obj, "persistent_id"):
            raise gc3libs.exceptions.LoadError(
                "Invalid format in file '%s':" " missing 'persistent_id' attribute" % (filename,)
            )
        if str(obj.persistent_id) != str(id_):
            raise gc3libs.exceptions.LoadError(
                "Retrieved persistent ID '%s' %s"
                " does not match given ID '%s' %s" % (obj.persistent_id, type(obj.persistent_id), id_, type(id_))
            )
    ...
    def _save_or_replace(self, id_, obj):
        ...
        if not os.path.exists(self._directory):
            try:
                ...
                gc3libs.log.error("Could not create jobs directory '%s': %s" % (self._directory, str(ex)))
                raise
        ...
        with open(filename, "w+b") as tgt:
            try:
                ...
                gc3libs.log.error(
                    "Error saving task '%s' to file '%s': %s: %s", obj, filename, err.__class__.__name__, err
                )
