...
if __name__ == "__main__":
    ...
    class SparseGridApplication(Application):
        ...
        def __init__(self, np, total_memory, fnum, dimension, lmax, epsilon, executable=None, **extra_args):
            """
    ...
    class SparseGridScript(SessionBasedScript):
        ...
        def __init__(self):
            # todo: which __version__
            SessionBasedScript.__init__(self, version=__version__)

        def setup_options(self):
            self.add_param(
                "--fnum", action="store", dest="fnum", default="1", help="Select function no. to test [1-6]."
            )
            self.add_param("--dimension", action="store", dest="dimension", default="2", help="Grid dimension.")
            self.add_param("--lmax", action="store", dest="lmax", default="3", help="Select Lmax")
            self.add_param("--epsilon", action="store", dest="epsilon", default="1.e-3", help="Select epsilon.")
            self.add_param(
                "--mpi_cores", action="store", dest="mpi_cores", default="10", help="Select number of cores to use. "
            )
