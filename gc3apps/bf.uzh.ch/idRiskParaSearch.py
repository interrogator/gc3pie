...
if __name__ == "__main__":
    ...
    if "-N" in sys.argv:
        ...
        if (
            "gc3IdRisk.csv" in filesAndFolder or "idRiskParaSearch.csv" in filesAndFolder
...
# Set up logger
logger = wrapLogger(
    loggerName="idRiskParaSearchLogger", streamVerb="DEBUG", logFile=os.path.join(os.getcwd(), "idRiskParaSearch.log")
)
...
class solveParaCombination(SequentialTaskCollection):
    ...
    def next(self, *args):
        ...
        if self.wBarLower_task.execution.returncode == 13:
            ...
            optimalRunFile.write(optimalRunTable)
            return Run.State.TERMINATED
        else:
...
class idRiskParaSearchDriver(SequentialTaskCollection):
    ...
    def next(self, *args):
        ...
        if not self.costlyOptimizer.checkConvergence():
            ...
            logger.debug(
                "converged idRiskParaSearchDriver.next in iteration %s for variables %s. Returning exit code 0. "
...
class idRiskParaSearchParallel(ParallelTaskCollection, forwardPremium.paraLoop_fp):
    ...
    def target(self, xVars, xParaCombos, targetVars, target_fx):
        ...
        """
        logger.debug(
            "entering idRiskParaSearchParallel.target. Computing target for xVar = %s, xVals = %s"
            % (xVars, xParaCombos)
        )
...
if __name__ == "__main__":
    ...
    if tableDicts:
        ...
        f = open(os.path.join(os.getcwd(), "optimalRuns"), "a+")
        f.write(optimalRuns)
        f.flush()
