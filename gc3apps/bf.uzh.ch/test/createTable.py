...
from pymods.support.support import getUniqueList, is_float, is_int, wrapLogger
...
def createOverviewTable(
    resultDir, outFile="simulation.out", exportFileName="overviewSimu", sortCols=[], orderCols=[], verb="DEBUG"
):
    """
  ...
  """
    ...
    for ixFolder, (resultFolder, pathToOutputFile) in enumerate(resultFolders):
        ...
        for ixLine, line in enumerate(lines):
            line = line.rstrip()
            if line.find("simulation ended") >= 0:
                continue
            if line:
                lineEles = line.rstrip().split(":")
                if lineEles[0] == "Iteration":
                    continue
                if re.search("iBar_Shock[2-9]Agent0", lineEles[0]):
                    continue
                if len(lineEles) != 2:
                    continue
