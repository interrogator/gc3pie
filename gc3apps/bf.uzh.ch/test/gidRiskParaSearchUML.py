...
# logger
from pymods.support.support import fillInputDir, wrapLogger
...
if __name__ == "__main__":
    ...
    if "-N" in sys.argv:
        ...
        if (
            "gc3IdRisk.csv" in filesAndFolder
            or "idRiskParaSearch.csv" in filesAndFolder
            or "gidRiskParaSearchUML.csv" in filesAndFolder
    ...
    if self.can_retrieve:
        ...
        for index, task in enumerate(self._terminating):
            ...
            try:
                ...
                gc3libs.log.error(
                    "Ignored error in fetching output of task '%s': %s: %s" % (task, x.__class__.__name__, str(x)),
                    exc_info=True,
                )
...
logger = wrapLogger(
    loggerName=curFileName + ".log", streamVerb="INFO", logFile=os.path.join(os.getcwd(), curFileName + ".log")
)
...
class solveParaCombination(SequentialTaskCollection):
    ...
    def next(self, *args):
        ...
        if self.wBarLower_task.execution.returncode == 13:
            ...
            wBarTable = tableDict.fromTextFile(
                os.path.join(self.paraFolder, "optimwBarLower", "overviewSimu"), width=20, prec=10
            )
            optimalRunTable = wBarTable.getSubset(
                np.abs(wBarTable["wBarLower"] - self.wBarLower_task.costlyOptimizer.best_x) < 1.0e-7
            )
...
class idRiskParaSearchDriver(SequentialTaskCollection):
    ...
    def next(self, *args):
        ...
        logger.debug("")
        logger.debug(
            "entering idRiskParaSearchDriver.next in iteration %s for variables %s and paraCombo %s"
            % (self.iter, self.solverParas["xVars"], self.jobname)
        )
        logger.debug("")
        ...
        if newVals is None:
            logger.critical("")
            logger.critical(
                "FAILURE: newVals is None. Evaluating variable %s at guess \n%s failed" % (self.xVars, self.xParaCombos)
            )
            logger.critical("")
            self.execution.returncode = 13
            self.failed = True
        ...
        if not self.costlyOptimizer.checkConvergence():
            ...
            self.evaluator = idRiskParaSearchParallel(
                self.xVars, self.xParaCombos, self.substs, self.optimFolder, self.solverParas, **self.sessionParas
            )
            self.add(self.evaluator)
        else:
            logger.debug("")
            logger.debug(
                "SUCCESS: converged idRiskParaSearchDriver.next in iteration %s for variables %s. Returning exit code 0. "
                % (self.iter, self.solverParas["xVars"])
            )
            logger.debug("")
            self.execution.returncode = 0
            return Run.State.TERMINATED
        logger.debug(
            "done idRiskParaSearchDriver.next in iteration %s for variables %s" % (self.iter, self.solverParas["xVars"])
        )
...
class idRiskParaSearchParallel(ParallelTaskCollection, paraLoop):
    ...
    def target(self, xVars, xParaCombos, targetVars, target_fx):
        ...
        """
        logger.debug(
            "entering idRiskParaSearchParallel.target. Computing target for xVar = %s, xVals = \n%s, targetVars = %s"
            % (xVars, xParaCombos, targetVars)
        )
        # Each line in the resulting table (overviewSimu) represents one paraCombo
        overviewTable = createOverviewTable(
            resultDir=self.optimFolder,
        ...
        for ixParaCombo, xParaCombo in enumerate(xParaCombos):
            ...
            for xVar, xVal in zip(xVars, xParaCombo):
                overviewTableSub = overviewTableSub.getSubset(np.abs(overviewTableSub[xVar] - xVal) < 1.0e-8)
            if len(overviewTableSub) == 0:
                logger.critical(
                    "Cannot find value for xVal %s, i.e. overviewTableSub empty. Did you set the pythonPath?" % xVal
                )
...
class idRiskParaSearchScript(SessionBasedScript, paraLoop):
    ...
    def parse_args(self):
        ...
        if os.path.isdir(self.params.executable):
            self.params.executable = os.path.join(self.params.executable, "idrisk")
...
def combineTables():
    ...
    tableList = [
        ...
        if os.path.isdir(folder) and folder != "localBaseDir" and folder != "idRiskParaSearch.jobs"
    ]
