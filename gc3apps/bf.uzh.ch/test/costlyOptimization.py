  ...
  """
    ...
    def updateInterpolationPoints(self, x, fx):
        ...
        if self.nUpdates == 0:
            ...
            self.logger.debug(
                "storing initial fx values %s corresponding to inital guess \n%s" % (fx, self.xInitialGuess)
            )
            ixParaCombo = 0
            indicesToDelete = []
            for paraCombo, paraComboFx in zip(x, fx):
                ...
                if not paraComboFx:
                    self.logger.debug(
                        "adding paraCombo = %s (element = %s) to indicesToDelete because of missing paraComboFx"
                        % (paraCombo, ixParaCombo)
                    )
        ...
        if nAboveTargetInt == 0:
            ...
            self.protected_ix = np.nonzero(aboveTarget)[0][
                0
            ]  # finds the first occurance of nonzero element in sorted array
        elif nBelowTarget == 0:
    ...
    def checkConvergence(self):
        ...
        if np.all(distance < self.convCrit):
            self.logger.debug(
                "SUCCESS: converged at fx %s with target %s to precision %s"
                % (self.best_fx, self.target_fx, self.convCrit)
            )
            self.converged = True
        else:
            self.logger.debug(
                "not converged at fx %s with target %s to precision %s" % (self.best_fx, self.target_fx, self.convCrit)
            )
            self.converged = False
    ...
    def updateApproximation(self):
        ...
        if len(self.x[0]) == 1:  # we are dealing with the one-dimensional case
            ...
            fIn = np.log((1.0 - self.target_fx) + fIn)
    ...
    def generateNewGuess(self):
        ...
        if np.all(self.fx - self.target_fx > 0) or np.all(self.fx - self.target_fx < 0):
            self.logger.critical(
                "the initial points %s with fx = \n%s do not contain the zero" % (self.xInitialGuess, self.fx)
            )
        self.logger.debug("Evaluating polynomial to find zero. ")
        xMat = np.linspace(self.optimRange[0], self.optimRange[1], 1.0e6)
        fxGrid = np.exp(self.gx(xMat)) - (1.0 - self.target_fx)
        ...
        topCandidatesBool = np.abs(sortedDistances - sortedDistances[0]) < 1.0e-4
    ...
    def plot(self):
        ...
        ax.grid()
        # plot approximation
        xVals = np.linspace(self.plotRange[0], self.plotRange[1], 1.0e3)
        try:
        ...
        ax.plot(
            self.sorted_x[self.intPoints_ix],
            self.sorted_fx[self.intPoints_ix],
            marker="o",
            markersize=2,
            linestyle="",
            color="r",
