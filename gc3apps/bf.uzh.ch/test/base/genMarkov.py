...
from pymods.markovChain.mcInterface import MkovM, mcInterface
from pymods.support.support import getParameter, myArrayPrint, wrapLogger
...
logger = wrapLogger(
    loggerName="genMarkovLog", streamVerb="DEBUG", logFile=os.path.join(os.getcwd(), "output/logs/genMarkov.log")
)


def genMarkov(markovFilePath, verb="INFO", nSimulation=int(5.0e4)):
    ...
    if nAgent > 1:
        ...
        ShockMatrixUnem = (
            np.triu(np.ones((nAgent, nAgent)), 1) * dy / (nAgent - 1)
            + np.tril(np.ones((nAgent, nAgent)), -1) * dy / (nAgent - 1)
            + np.eye(nAgent) * (-dy)
        )
        ...
        fullTransMatrix = np.vstack(
            (
                np.hstack((np.eye(nAgent) * pers, np.ones((nAgent, 1)) * (1.0 - pers))),
                np.append(((1.0 - pers) / nAgent) * np.ones(nAgent), pers),
            )
        )
    else:
    ...
    PD = 1.0 - (1.0 - theta) * np.reshape(Etg, (len(Etg), 1)) / g
    ...
    varSim["incGrIndAg0"] = float(1.0)
    varSim["incShareAg0"] = (1.0 + varSim["dy_agent1"]) / nAgent

    for row in varSim.rows():
        if row > 0:
            varSim["simInc"][row] = varSim["simInc"][row - 1] * varSim["agIncGrowth"][row]
            varSim["incGrIndAg0"][row] = (varSim["incShareAg0"][row] / varSim["incShareAg0"][row - 1]) * varSim[
                "agIncGrowth"
            ][row]
