...
class wrapLogger:
    def __init__(
        ...
        logFile="logFile",
        fileVerb="DEBUG",
        streamFormat="{record.message}",
        fileFormat="{record.message}",
    ):
    ...
    def __setstate__(self, state):
        self.__dict__ = state
        logger = getLogger(
            loggerName=self.loggerName, streamVerb=self.streamVerb, logFile=self.logFile, fileVerb="DEBUG"
        )
        self.wrappedLog = logger
    ...
    def setStreamVerb(self, verb):
        ...
        for handler in self.wrappedLog.handlers:
            if type(handler) != logbook.StreamHandler:
                newHandlers.append(handler)
        # create new handler with adjusted verbosity
        self.streamVerb = verb
        mySH = logbook.StreamHandler(
            stream=sys.stdout, level=self.streamVerb.upper(), format_string=self.streamFormat, bubble=True
        )
    ...
    def setFileVerb(self, verb):
        ...
        # take out previous stream handlers
        for handler in self.wrappedLog.handlers:
            if type(handler) != logbook.FileHandler:
                newHandlers.append(handler)
        # create new handler with adjusted verbosity
        self.fileVerb = verb
