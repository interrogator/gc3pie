...
class tableDict:
  ...
  """
    ...
    """
        ...
        self.cols = np.array(
            cols
        )  # list(cols) # self.cols needs to be list since I use append in various methods. (not np.array(cols))
        self.data = dict()
        if not dataList:
            self.nRows = 0
            self.colFormat = {}
...
if __name__ == "__main__":
    ...
    myTable = tableDict.fromTextFile(
        "/home/benjamin/workspace/fpProj/empirical/outputInput/momentTable/Gdp/gdpMoments.csv", ","
    )
    myTable.setWidth(7)
