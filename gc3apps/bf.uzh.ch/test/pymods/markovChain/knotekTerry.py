...
def fn_var_to_markov(A0, A1, A2, SIGMA, N, random_draws, method):
  ...
  """
    ...
    for i in range(len(N) - 2, -1, -1):
        Pr_mat_key[i, :] = numpy.tile(
            numpy.kron(zbar[i, : N[i]], numpy.ones((1, numpy.prod(N[i + 1 : len(N)])))),
            (1, numpy.prod(N) / numpy.prod(N[i : len(N)])),
        )
        Pr_mat_key_pos[i, :] = numpy.tile(
            numpy.kron(numpy.arange(N[i]), numpy.ones((1, numpy.prod(N[i + 1 : len(N)])))),
            (1, numpy.prod(N) / numpy.prod(N[i : len(N)])),
        )

    nstate = numpy.prod(N)
    Pr_mat_intervals = numpy.zeros(
        (n, nstate, 2)
    )  # this will store the unadjusted limits of integration for each variable in each state, for input into the Genz code
    if method == 1:
        ...
        for i in range(nstate):  #  number of states
            for j in range(n):  # number of variables
                if Pr_mat_key_pos[j, i] == 0:
                    Pr_mat_intervals[j, i, 0] = -numpy.inf
                    Pr_mat_intervals[j, i, 1] = (
                        zbar[j, Pr_mat_key_pos[j, i]]
                        + (zbar[j, Pr_mat_key_pos[j, i] + 1] - zbar[j, Pr_mat_key_pos[j, i]]) / 2
                    )
                elif Pr_mat_key_pos[j, i] == N[j] - 1:
                    Pr_mat_intervals[j, i, 0] = (
                        zbar[j, Pr_mat_key_pos[j, i]]
    ...
    for i in range(nstate):  # ; rows of Pr_mat
        Pr_mat_intervals_adjusted[:, :, 0] = Pr_mat_intervals[:, :, 0] - numpy.tile(
            (A1bar.T + numpy.dot(A2bar, Pr_mat_key[:, i])).T, (1, nstate)
        )
        Pr_mat_intervals_adjusted[:, :, 1] = Pr_mat_intervals[:, :, 1] - numpy.tile(
            (A1bar.T + numpy.dot(A2bar, Pr_mat_key[:, i])).T, (1, nstate)
        )
        for j in range(nstate):  # columns of Pr_mat
            # Pr_mat(i,j) = P(state j|state i)
            [Pr_mat[i, j], error_est[i, j]] = qscmvnv(
                random_draws,
                SIGMAbar,
                Pr_mat_intervals_adjusted[:, j, 0],
                numpy.eye(n),
                Pr_mat_intervals_adjusted[:, j, 1],
            )
...
def qscmvnv(m, r, a, cn, b):
  ...
  """
    ...
    q = 2 ** ((numpy.arange(n) + 1) / (n + 1.0))  # Niederreiter point set generators
    #
    ...
    for i in range(ns):
        ...
        xx[:, : nv + 1] = numpy.abs(
            2
            * numpy.mod(
                numpy.kron(q.reshape(q.shape[0], -1), numpy.arange(1, nv + 1))
                + numpy.random.rand(n, 1) * numpy.ones(nv),
                1,
            )
            - 1
        )
        vp = mvndnv(n, asVar, ch, bs, clg, ci, dci, xx, nv)
        vp = (mvndnv(n, asVar, ch, bs, clg, ci, dci, 1 - xx, nv) + vp) / 2.0  # symmetrize
        d = (numpy.mean(vp) - p) / (i + 1)
        p = p + d
        if numpy.abs(d) > 0:
            e = numpy.abs(d) * numpy.sqrt(1.0 + (e / d) ** 2 * (i - 1) / (i + 1))
        else:
            if i > 1:
