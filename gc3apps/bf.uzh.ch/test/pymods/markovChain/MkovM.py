...
class MkovM:
    ...
    def getV(self):
        ...
        for ixVar1 in range(self.vars):
            for ixVar2 in range(self.vars):
                V[ixVar1, ixVar2] = np.sum(
                    self.lmbda * (self.S[:, ixVar1] - self.E[ixVar1]) * (self.S[:, ixVar2] - self.E[ixVar2])
                )
        return V
    ...
    def getCor(self):
        ...
        for ixVar1 in range(self.vars):
            for ixVar2 in range(self.vars):
                ...
                if (Std[ixVar1] < 1.0e-10) or (Std[ixVar2] < 1.0e-10):
    ...
    def getSkew(self):
        ...
        for ixVar in range(self.vars):
            ...
            skew[ixVar] = mu3 / self.V[ixVar, ixVar] ** (3.0 / 2.0)
    ...
    def getVEt(self):
        ...
        for ixVar1 in range(self.vars):
            for ixVar2 in range(self.vars):
                VEt[ixVar1, ixVar2] = np.sum(
                    self.lmbda * (Et[:, ixVar1] - self.E[ixVar1]) * (Et[:, ixVar2] - self.E[ixVar2])
                )
                if VEt[ixVar1, ixVar2] < -1.0e-4:  # Take some slack here.
                    print("what?")
        return VEt
    ...
    def getVt(self):
        ...
        for ixState in range(self.states):
            for ixVar1 in range(self.vars):
                for ixVar2 in range(self.vars):
                    condV[ixVar1, ixVar2] = np.dot(
                        self.T[ixState, :],
                        (self.S[:, ixVar1] - self.Et[ixState, ixVar1]) * (self.S[:, ixVar2] - self.Et[ixState, ixVar2]),
                    )
                    if condV[ixVar1, ixVar2] < -1.0e-4:
                        print("what?")
            Vt.append(condV)
        return Vt
    ...
    def getStdt(self):
        ...
        Vt = self.getVt()
        for ixState in range(self.states):
            Stdt.append(np.diag(Vt[ixState]) ** (1.0 / 2.0))
        return Stdt
    ...
    def getStdEt(self):
        ...
        for ixVar1 in range(self.vars):
            for ixVar2 in range(self.vars):
                StdEt[ixVar1, ixVar2] = VEt[ixVar1, ixVar2] ** (1.0 / 2.0)
        return StdEt
    ...
    """
        ...
        for ixVar1 in range(self.vars):
            for ixVar2 in range(self.vars):
                CovSEtS[ixVar1, ixVar2] = np.sum(
                    self.lmbda * (self.S[:, ixVar1] - self.E[ixVar1]) * (Et[:, ixVar2] - self.E[ixVar2])
                )
        return CovSEtS
...
if __name__ == "__main__":
    ...
    nStates = np.size(S, 0)
    T = 1.0 / nStates * np.ones((nStates, nStates))
