...
import tempfile
import filecmp
from .support.getIndex import getIndex
from .support.support import flatten, str2tuple, lower
from .support.log import logger
from optparse import OptionParser
import sys, os, shutil, time, datetime, subprocess, threading, queue
import distutils.dir_util, glob
from .support.log import initialize_logging
...
def loop(args=None):
    ...
    parser = OptionParser()
    parser.add_option(
        "-f", "--folder", dest='confFolder', action='store', help='Specify folder containing config files'
    )
    parser.add_option(
        "-d", "--dryrun", dest='dryrun', action="store_true", default=False, help="Take the loop for a test run"
    )
    if args:
    ...
    dt = dtype = {
        'names': ['variables', 'indices', 'paraFiles', 'paraProps', 'groups', 'groupRestrs', 'vals', 'paraFileRegex'],
    ...
    nParams = len(params)
    print('number of read parameters:', nParams, file=withLog)
    ...
    print('variables \n', variables, file=withLog)
    print('indices \n', indices, file=withLog)
    print('vals \n', vals, file=withLog)
    print('paraFiles \n', paraFiles, file=withLog)
    print('groups\n', groups, file=withLog)
    print('groupRestrs\n', groupRestrs, file=withLog)
    print('paraFileRegex \n', paraFileRegex, file=withLog)
    ...
    for paraFile in paraFiles:
        if paraFile in tmpParaFiles:
            continue
        tmpParaFiles[paraFile] = tempfile.NamedTemporaryFile(
            mode='w', suffix='', prefix='tmp', dir=loopTmpFolder, delete=False
        ).name
    ...
    dateString = '{:04d}-{:02d}-{:02d}-{:02d}-{:02d}-{:02d}'.format(
    ...
    for ixVar, var in enumerate(variables[groups >= 0]):
        ...
        if ixVar == len(variables[groups >= 0]) - 1:
            ...
            resultsFolder += var + ','
    print('\nResultsfolder')
    print(resultsFolder, file=withLog)
    if not opts['dryrun']:
    ...
    for ixGroup, group in enumerate(np.unique(groups[groups >= 0])):
        ...
        print('groupIndices \n', groupIndices[ixGroup], file=withLog)
    ...
    print('\n\n')
    print('---------', file=withLog)
    print('Summary after establishing groups: ')
    print('groupbase', groupBase, file=withLog)
    print('groupindices', groupIndices, file=withLog)
    print('metabase', metaBase, file=withLog)
    print('metaind', metaIndices, file=withLog)
    print('---------', file=withLog)
    print('\n')
    ...
    print('Queue has size:', exeQueue.qsize())
    print('\n')

    # sys.exit()

    print('-----Main loop started------', file=withLog)
    for ixMeta, meta in enumerate(metaIndices):
        print('--\n\n--', file=withLog)
        # print('Main iteration(ixMeta): ', ixMeta, file = withLog)
        print(
            'Loop iteration(ixMeta)',
            ixMeta,
            ' out of ',
            nMetaIndices,
            ' ( ',
            (ixMeta / nMetaIndices) * 100,
            '% )',
            file=withLog,
        )
        ...
        print('index b4 flattening', index)
        index = list(flatten(index))
        print('index', index, file=withLog)
        for ixVar in range(0, len(variables)):
            print('variable nr', ixVar, 'var', variables[ixVar])

        runDescription = ''
        for ixVar, var in enumerate(variables):
            print('\nvariable', variables[ixVar], file=withLog)
            ...
            print('paraIndex', paraIndex, file=withLog)
            newValMat = updateParameter(
                fileIn=tmpParaFiles[paraFile], varIn=var, paraIndex=paraIndex, newVal=val, regexIn=regex
            )
            print('newValMat', newValMat)
            if group >= 0 and paraProps[ixVar] != 'swIndicator':
                if ixVar < len(variables):
                    runDescription += '_'
                runDescription += '{0}={1}'.format(var, newValMat.replace(' ', ''))
        print('\nrunDescription', runDescription, file=withLog)
        ...
        if not opts['dryrun']:
            computeSolution(exe, loopConf['saveFiles'], tmpParaFiles, paraFiles, loopTmpFolder, folderName, exeQueue)
    ...
    if not opts['dryrun']:
        withLog.saveLogFile(resultsFolder)
    ...
    for ixGroup, group in enumerate(np.unique(groups)):
        ...
        if group != -1:
            ...
            for groupVarIndex in groupVarIndices:
                ...
                if not paraProp == 'swIndicator':
                    ...
                    if (
                        ixMeta > 0 and groupIndices[group][metaIndices[ixMeta - 1][group]] != groupIndex
                    ):  # group has changed indices
        ...
        #  index.append(groupIndices[group][meta[group]])
            ...
            if sum(groupMinus1) > 1:
                print('warning... more than one -1 variable not supported')
...
def computeSolution(exe, saveFiles, tmpParaFiles, paraFiles, loopTmpFolder, folderName, exeQueue, outStream=sys.stdout):
    ...
    print('call: ', mainExecutable, file=withLog)
    ...
    for ixSaveFile, relPsaveFile in enumerate(relPsaveFiles):
        if os.path.isdir(absPsaveFiles[ixSaveFile]):
            ...
            distutils.dir_util.copy_tree(
                os.path.join(inFolderName, relPsaveFile), os.path.join(outFolderName, relPsaveFile)
            )
...
if __name__ == '__main__':
    ...
    args = ['-f', '/mnt/shareOffice/ForwardPremium/generateResults/Loop/Ben/']
    loop()
    print('\ndone')
