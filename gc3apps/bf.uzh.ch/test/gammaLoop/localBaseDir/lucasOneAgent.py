...
def CRRA(c, gamma=2):
    return 1.0 / (1.0 - gamma) * c ** (1.0 - gamma)
...
def eulerCRRA(g, beta, gamma, transMatrix, payoff):
    if payoff.ndim > 1:
        euler = beta * inner1d(transMatrix, g ** (1.0 - gamma) * (payoff))
    else:
        euler = beta * np.dot(transMatrix, g ** (1.0 - gamma) * (payoff))
    return euler


def EpsteinZin(c, g, V, beta, gamma, psi, transMatrix):
    rho = 1.0 / psi
    newV = (c ** (1 - rho) + beta * np.dot(transMatrix, (g * V) ** (1.0 - gamma)) ** ((1.0 - rho) / (1.0 - gamma))) ** (
        1.0 / (1.0 - rho)
    )
    return newV
...
def eulerEpsteinZin(g, V, beta, gamma, psi, transMatrix, payoff):
    ...
    if payoff.ndim > 1:
        euler = (
            beta
            * (np.dot(transMatrix, (g * V) ** (1.0 - gamma))) ** ((gamma - rho) / (1.0 - gamma))
            * inner1d(transMatrix, g ** (1.0 - gamma) * V ** (rho - gamma) * payoff)
        )
    else:
        euler = (
            beta
            * (np.dot(transMatrix, (g * V) ** (1.0 - gamma))) ** ((gamma - rho) / (1.0 - gamma))
            * np.dot(transMatrix, g ** (1.0 - gamma) * V ** (rho - gamma) * payoff)
        )
    return euler
