...
from supportGc3 import extractVal, flatten, format_newVal, getIndex, lower, str2tuple, str2vals, wrapLogger
...
class paraLoop(object):
    ...
    def process_para_file(self, path_to_para_loop):
        ...
        for ixGroup, group in enumerate(np.unique(groups[groups >= 0])):
            ...
            for ix in range(len(relevGroups)):
    ...
    def getFullIndex(ixMeta, metaIndices, groupIndices, groups, paraProps, vals):
        ...
        for ixGroup, group in enumerate(np.unique(groups)):
            ...
            if group != -1:
                ...
                # todo: this does not work because it is a static method...
    ...
    def _remap_groups(self, groups):
        ...
        return np.apply_along_axis(np.vectorize(remap_group_nrs), 0, groups)
    ...
    def writeParaLoop(
        ...
        paraFileRegex=None,
        vals=None,
        desPath="para.loopTmp",
    ):
        paraLoopFile = open(desPath, "a+")
        paraLoopFile.write(
