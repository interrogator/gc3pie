    ...
    def __init__(
        ...
        if self.show_progress:
            self.evaluator.print_status(
                self.scores.min(), self.scores.mean(), self.population[self.scores.argmin()], "Final"
            )
        ...
        while not converged:
            ...
            if self.show_progress:
                if self.count % self.show_progress_nth_cycle == 0:
                    ...
                    self.evaluator.print_status(
                        self.scores.min(), self.scores.mean(), self.population[self.scores.argmin()], self.count
                    )
...
class test_rosenbrock_function(object):
    def __init__(self, dim=5):
        ...
        self.optimizer = differential_evolution_optimizer(
            self, population_size=min(self.n * 10, 40), n_cross=self.n, cr=0.9, eps=1e-12, show_progress=True
        )
        for x in self.x:
