...
from gc3libs.workflow import ParallelTaskCollection, SequentialTaskCollection, StagedTaskCollection
...
class GenREMDatasetApplication(Application):
    ...
    def __init__(self, method, data_file_list, source_folder, **extra_args):
        ...
        for data_folder in data_file_list:
            for data_file in os.listdir(data_folder):
                inputs[os.path.join(data_folder, data_file)] = os.path.join(REMOTE_DATA_FOLDER, data_file)

        for script_file in os.listdir(source_folder):
            inputs[os.path.join(source_folder, script_file)] = os.path.join(
                REMOTE_SCRIPTS_FOLDER, os.path.basename(script_file)
            )

        arguments = RSCRIPT_COMMAND.format(method=method, src=REMOTE_SCRIPTS_FOLDER, data=REMOTE_DATA_FOLDER)
    ...
    def terminated(self):
        ...
        for data in os.listdir(os.path.join(self.output_dir, REMOTE_RESULT_FOLDER)):
            shutil.move(os.path.join(self.output_dir, REMOTE_RESULT_FOLDER, data), os.path.join(self.output, data))
...
class GenREMStagedTaskCollection(StagedTaskCollection):
    ...
    def stage2(self):
        ...
        return GenREMDatasetApplication(
            "merge", [self.s0_outputfolder, self.s1_outputfolder], self.source_folder, **extra_args
        )
...
class GenREMScript(SessionBasedScript):
    ...
    def __init__(self):
        SessionBasedScript.__init__(
            self, version=__version__, application=GenREMDatasetApplication, stats_only_for=GenREMDatasetApplication
        )

    def setup_args(self):

        self.add_param("data_folder", type=existing_directory, help="Location of initial RData file.")
    ...
    def setup_options(self):
        ...
        self.add_param("-R", "--results", metavar="PATH", dest="result", default="results", help="Location of results.")
    ...
    def new_tasks(self, extra):
        ...
        return [GenREMStagedTaskCollection(self.params.data_folder, self.params.src, **extra_args)]
