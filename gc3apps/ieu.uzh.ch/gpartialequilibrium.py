...
class GpartialequilibriumApplication(Application):
    ...
    def __init__(self, input_file, mfunct, **extra_args):
        ...
        wrapper = resource_filename(Requirement.parse("gc3pie"), "gc3libs/etc/matlab_wrapper.sh")
    ...
    def terminated(self):
        ...
        for f in os.listdir(folder_files):
            os.rename(os.path.join(folder_files, f), os.path.join(self.results, f))
...
class GpartialequilibriumScript(SessionBasedScript):
    ...
    def new_tasks(self, extra):
        ...
        for csv_input_file in self.params.args:
            for (input_file, index_chunk) in _getchunk(
                csv_input_file,
                self.params.chunk_size,
                chunk_files_dir=os.path.join(self.session.path, "tmp", os.path.basename(csv_input_file)),
            ):
                ...
                extra_args["output_dir"] = extra_args["output_dir"].replace(
                    "NAME", os.path.join(os.path.basename(csv_input_file), jobname)
                )
                ...
                tasks.append(GpartialequilibriumApplication(input_file, self.params.mfunct, **extra_args))
