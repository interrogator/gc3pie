...
class GMhcCoevTask(SequentialTaskCollection):
        ...
        **extra_args
        ...
        if "jobname" in extra_args:
            ...
            if self.executable is None:
                self.jobname = "MHC_coev.%s.%s.%s.%s.%s" % (N, p_mut_coeff, choose_or_rand, sick_or_not, off_v_last)
            else:
    ...
    def next(self, done):
        ...
        exclude = [os.path.basename(self.tasks[done].arguments[0]), stdout_filename, stderr_filename]
        ...
        if last_run.duration < 5 * minutes:
            report.write(
                "gmhc_coev: Job %s did not run at all," " will try resubmitting to a different resource.\n" % (self,)
            )
            # resubmit
            return done
        ...
        # 1. out of memory?
            ...
            or (last_run.exitcode == 255 and ("Out of memory." in stderr_contents or "MATLAB:nomem" in stderr_contents))
        ):
            self.extra["requested_memory"] = _increase(
                self.extra["requested_memory"], 8 * GB, increment=2 * GB
            )  # start growing linearly at 8GB
            report.write(
        ...
        # 2. not enough time to compute even one generation?
            ...
            self.single_run_duration = _increase(self.single_run_duration, 24 * hours, increase=4 * hours)
            report.write(
        ...
        if not os.path.exists(latest_work):
            latest_work = None
            report.write("gmhc_coev: No `latest_work.mat` file was produced," " next job will start from k=1.\n")
        if self.generations_done < self.generations_to_do:
            ...
            gc3libs.log.debug("Computed %s generations, no more to do." % (self.generations_done))
            report.write("gmhc_coev:" " Computed %s generations, no more to do.\n" % (self.generations_done,))
            report.close()
...
newly-created jobs so that this limit is never exceeded.
        ...
        for path in self.params.args:
            if path.endswith(".csv"):
                ...
                try:
                    ...
                    self.log.warning(
                        "Could not determine field delimiter in file '%s'," " assuming it's a comma character (',').",
                        path,
                    )
                    csv_dialect = "excel"
                inputfile.seek(0)
                for lineno, row in enumerate(csv.reader(inputfile, csv_dialect)):
                    ...
                    try:
                        ...
                        self.log.error(
                            "Wrong format in line %d of file '%s':"
                    ...
                    try:
                        ...
                        self.log.warning("Ignoring line '%s' in input file '%s': %s", str.join(",", row), path, str(ex))
                        continue
                    basename = "MHC_coev_" + GMhcCoevScript._params_to_str(
    ...
    def _choose_or_rand_to_str(choose_or_rand):
        if choose_or_rand == 1:
            ...
            raise ValueError("Valid values for `choose_or_rand` are: 1,2,3;" " got '%s' instead" % choose_or_rand)
    ...
    def _off_v_last_to_str(off_v_last):
        if off_v_last < 0.0 or off_v_last > 1.0:
            raise ValueError("Parameter `off_v_last` must be in range 0.0 to 1.0;" " got `%s`instead." % off_v_last)
        if off_v_last == 1.0:
            return "offval_1"
        else:
            # off_v_last == 0.xxxx
