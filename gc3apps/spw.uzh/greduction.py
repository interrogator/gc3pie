...
class GreductionApplication(Application):
    ...
    def __init__(self, language_file, **extra_args):
        ...
        Application.__init__(
            self, arguments=arguments, inputs=inputs, outputs=outputs, stdout='greduction.log', join=True, **extra_args
        )
...
class GreductionScript(SessionBasedScript):
    ...
    def setup_args(self):
        ...
        self.add_param('language_file', type=str, help="Input language file (in JSON format).")
    ...
    def parse_args(self):
        ...
        if not os.path.isfile(self.params.language_file):
            raise gc3libs.exceptions.InvalidUsage(
                "Invalid input language file: '%s'. File not found" % self.params.language_file
            )
    ...
    def new_tasks(self, extra):
        ...
        for (input_file, index_chunk) in self._generate_chunked_files_and_list(
            self.params.language_file, self.params.chunk_size
        ):
            ...
            tasks.append(GreductionApplication(input_file, **extra_args))
    ...
    def _generate_chunked_files_and_list(self, file_to_chunk, chunk_size):
        ...
        for index in range(0, len(data), chunk_size):
            (handle, tmp_filename) = tempfile.mkstemp(
                dir=chunk_files_dir, prefix='greduction-', suffix="%d.json" % index
            )
