...
class GBeastScript(SessionBasedScript):
    ...
    def parse_args(self):
        if not os.path.isdir(self.params.path):
            raise gc3libs.exceptions.InvalidUsage("{0} is not a directory".format(self.params.path))
    ...
    def new_tasks(self, extra):
        ...
        for dirname in self.params.path:
            if self.params.beast:
                ...
                if len(xmls) > 1:
                    self.log.error(
                        "More than 1 `.xml` file in directory `{dirname}`: {xmls!r}" " Skipping it.".format(**locals())
                    )
                    continue
                xml = xmls[0]
                if "BEAST1" in xml:
                    ...
                    gc3libs.error(
                        ...
                        " Skipping file `{xml}`".format(xml=xml)
                    )
                    continue
            jarfile = self.params.beast1 if beast == "beast1" else self.params.beast2
            tasks.append(GBeastApp(jarfile, dirname, self.params.cores, **extra.copy()))
        return tasks
