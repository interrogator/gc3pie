...
class GqhgApplication(Application):
    ...
    def __init__(self, qdf_file, seed, steps, **extra_args):
        ...
        Application.__init__(
            ...
            arguments=arguments,
            inputs=inputs,
            outputs=[DEFAULT_REMOTE_OUTPUT_FOLDER, output_logfile + ".log", output_logfile + ".out"],
            stdout="gqhg.log",
            join=True,
            **extra_args
        )
...
class GqhgScript(SessionBasedScript):
    ...
    def setup_options(self):
        ...
        self.add_param("-G", "--grid", metavar="[PATH]", dest="grid", default=None, help="Location of the Grid file")
        ...
        self.add_param(
            ...
            help="Repeat simulation on same .qdf file. " "Default: %(default)s.",
        )
        ...
        self.add_param(
            ...
            help="Number of steps within single simulation " "Default: %(default)s.",
        )
        ...
        self.add_param(
            ...
            dest="master",
            default=None,
            help="Use alternative binanry file. " "Note: binary has to be statically compiled",
        )
    ...
    def parse_args(self):
        ...
        try:
            ...
            if self.params.grid:
                assert os.path.isfile(self.params.grid), "Grid file %s not found" % self.params.grid

            self.qdf_files = [
                os.path.abspath(os.path.join(self.params.input_data, qdf))
                for qdf in os.listdir(self.params.input_data)
                if qdf.endswith(QDF_PATTERN)
            ]
            assert len(self.qdf_files) > 0, "No QDF files found in input folder %s" % self.params.input_data
    ...
    def new_tasks(self, extra):
        ...
        for qdf_file in self.qdf_files:
            ...
            for iteration in range(1, int(self.params.iterations) + 1):
                ...
                tasks.append(GqhgApplication(qdf_file, seed, int(self.params.steps), **extra_args))
