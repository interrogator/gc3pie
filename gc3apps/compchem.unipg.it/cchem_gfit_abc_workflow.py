...
class MainSequentialABC(SequentialTaskCollection):
    ...
    def next(self, done):
        ...
        if done == 0:
            ...
            second_task = ParallelABC(
                ABC_RUN, abc_executable, self.inputfilelist_abc, first_task.output_dir, **self.extra_args
            )
        ...
        **extra_args
        ...
        if surface_file:
            ...
            raise gc3libs.exceptions.InvalidArgument(
                "Missing critical argument surface file [%s], g3c_file [%s], dimension_file [%s]"
                % (surface_file, g3c_input_file, dimension_file)
            )
...
class ABC_Application(Application):
    ...
    def __init__(self, executable, abc_executable, input_file, output_folder, **extra_args):
        ...
        gc3libs.Application.__init__(
            ...
            inputs=[
                (abc_executable, os.path.basename(abc_executable)),
...
class ABC_Workflow(SessionBasedScript):
    ...
    def setup_options(self):
        ...
        self.add_param(
            "--g3c", action="store", dest="g3cfile", type=existing_file, default=None, help="G3C input file."
        )

        self.add_param(
            "--dim", action="store", dest="dimensions", type=existing_file, default=None, help="Surface file."
        )
        self.add_param(
            "--exec", action="store", dest="abc_exec", type=executable_file, default=None, help="ABC binary file."
        )
        self.add_param(
            "--pes",
            action="store",
    ...
    def new_tasks(self, extra):
        ...
        if self.params.dimensions and self.params.g3cfile:
            ...
            yield (
                ...
                extra_args,
            )  # MainSequentialABC(ABC_URL_IMAGE, self.apppot_run, self.params.out, ..., **extra_args)
