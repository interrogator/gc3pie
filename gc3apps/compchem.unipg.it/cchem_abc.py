...
from gc3libs import Application, RetryableTask, Run, Task
...
class ABCWorkflow(SessionBasedScript):
    ...
    def parse_args(self):
        ...
        for subst in self.params.subst:
            try:
                ...
                raise gc3libs.exceptions.InvalidUsage(
                    ...
                    " where LOW, HIGH and STEP are (integer or floating-point) numbers." % (subst,)
                )
        ...
        gc3libs.utils.test_file(self.abc_executable, os.R_OK | os.X_OK, gc3libs.exceptions.InvalidUsage)
        ...
        for path in self.params.args[1:]:
            ...
            if os.path.isdir(path):
                raise gc3libs.exceptions.InvalidUsage(
                    "Argument '%s' should be a file, but is a directory instead." % (path,)
                )
    ...
    def new_tasks(self, extra):
        ...
        for values in itertools.product(*self.subst_values):
            subst = dict((name, value) for name, value in zip(self.subst_names, values))
            jobname = str.join("_", [basename] + [("%s=%s" % (name, value)) for name, value in subst.iteritems()])

            if os.path.isdir(jobname):
                # assume job has already been created
                gc3libs.log.info("Directory '%s' already exists;" " assuming job has already been created.", jobname)
            else:
                ...
                extra_args["output_dir"] = os.path.join(self.make_directory_path(self.params.output, jobname), "output")
                if self.params.retry is not None:
                    yield (
                        jobname,
                        RetryableTask,
                        [jobname, ABCApplication(*inputs, **extra_args), self.params.retry],
                        extra_args,
                    )
                else:
