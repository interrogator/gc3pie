...
class GzodsApp(gc3libs.Application):
    ...
    def __init__(self, filename, version=DEFAULT_ZODS_VERSION, **extra_args):
        if self.check_input(filename) is None:
            raise gc3libs.exceptions.InputFileError(
                ...
                " and -optionally- the restart file.)" % (filename,)
            )
    ...
    def check_input(self, filename):
        ...
        if (
            len(cNodes[0].getElementsByTagName('reference_intensities')) > 0
            and len(cNodes[0].getElementsByTagName('average_structure')) > 0
        ):
            data_file = cNodes[0].getElementsByTagName('reference_intensities')[0].getAttribute('file_name')
            avg_file = (
                cNodes[0]
                .getElementsByTagName('average_structure')[0]
            ...
            gc3libs.log.debug(
                "gzods.py: %s references to the following files: %s and  %s.", filename, avg_file, data_file
            )
            if os.path.exists(avg_file) == False or os.path.exists(data_file) == False:
                gc3libs.log.warning(
                    "gzods.py: Averaged structure file %s or reference intensities file %s DO NOT exist.",
                    avg_file,
                    data_file,
                )
                ...
                gc3libs.log.info(
                    "Input file '%s' references" " averaged structure file '%s'" " and reference intesities file '%s'.",
                    filename,
                    avg_file,
                    data_file,
                )
                ...
                if len(cNodes[0].getElementsByTagName('run_type')) > 0:
                    ...
                    if len(cNodes[0].getElementsByTagName('optimization_method')) > 0:
                        ...
                        if len(cNodes[0].getElementsByTagName('restart')) > 0:
                            ...
                            restart_file = (
                                cNodes[0]
                                .getElementsByTagName('optimization_method')[0]
                                .getElementsByTagName('restart')[0]
                                .getAttribute('file')
                            )
                            ...
                            if os.path.exists(restart_file) == False:
                                raise gc3libs.exceptions.InputFileError(
                                    "gzods.py: Input file '%s' references a file '%s' that DOES NOT exists."
                                    % (filename, restart_file)
                                )
                            else:
                                gc3libs.log.info(
                                    "gzods.py: %s references an optional restart file: %s.", filename, restart_file
                                )
  ...
  ./gzods input.xml -c 5
    ...
    def setup_options(self):
        self.add_param(
            ...
            metavar="VERNO",
            dest="verno",
            default=DEFAULT_ZODS_VERSION,
            help="Request the specified version of ZODS" " (default: %(default)s).",
        )
