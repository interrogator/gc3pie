...
class SmartApplication(Application):
    ...
    def __init__(self, arguments, inputs=None, *more_args, **extra_args):
        ...
        if inputs is not None:
            inputs = Application._io_spec_to_dict(gc3libs.url.UrlKeyDict, inputs, force_abs=True)
        else:
...
to the remote system as the command to be executed.
    ...
    def setup_args(self):
        self.add_param("cmd", metavar="CMD", type=executable_file, help="Path to the command to execute.")
        self.add_param("args", nargs="*", metavar="ARG", help="Arguments for the command to run.")
        ...
        for define in self.params.define:
            try:
                ...
                raise gc3libs.exceptions.InvalidUsage(
                    ...
                    " Parameter substitutions must have the form 'NAME=KIND:VALUE'." % (define,)
                )
            kind = kind.lower()
            # (1) file name
            if "from" == kind:
                gc3libs.utils.test_file(value, os.R_OK, exception=gc3libs.exceptions.InvalidUsage)
                self.subst[name] = gc3libs.utils.stripped(open(value, "r"))
                gc3libs.log.info("Reading values for %s from file '%s'", name, value)
            # (2) list of values
            elif "list" == kind:
                self.subst[name] = value.split(",")
                gc3libs.log.info("Parameter %s takes values: %s", name, self.subst[name])
            # (3) range
            elif "range" == kind:
                if value.count(":") == 2:
                    ...
                    raise gc3libs.exceptions.InvalidUsage(
                        ...
                        " 'NAME=range:LOW:HIGH:STEP'." % (define,)
                    )
                ...
                gc3libs.log.info(
                    "Parameter %s ranges from %s (incl.) to %s (excl.) in increments of %s", name, low, high, step
                )
    ...
    def new_tasks(self, extra):
        ...
        for values in itertools.product(*(self.subst[name] for name in names)):
            ...
            jobname = str.join(
                ...
                [basename]
                + [
                    ("%s=%s" % (name, value.translate(None, r'\/&|=%$#!?<>()`"' + r"'" + "\a\b\n\r")))
            ...
            if self.params.retry is not None:
                yield RetryableTask(
                    SmartApplication(arguments, inputs, **extra_args), self.params.retry, jobname=jobname, **extra_args
                )
            else:
                yield SmartApplication(arguments, inputs, jobname=jobname, **extra_args)
