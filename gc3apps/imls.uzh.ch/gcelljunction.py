...
class GCellJunctionApplication(Application):
    ...
    def __init__(self, sim_no, executable=None, restart=None, **extra_args):
        self.sim_no = sim_no
        wrapper_sh = resource_filename(Requirement.parse("gc3pie"), "gc3libs/etc/gcelljunction_wrapper.sh")
    ...
    def find_restart_data(output_dir):
        ...
        if os.path.isdir(data_dir):
            ...
            entries = [name for name in os.listdir(data_dir) if name.startswith('junction_') and name.endswith('.mat')]
            latest = None
            latest_idx = 0
            for name in entries:
    ...
    def update_state(self, **extra_args):
        ...
        if self.execution.state == Run.State.RUNNING:
            try:
                ...
                gc3libs.log.warning(
                    "Ignored error while updating state of Task %s: %s: %s", self, err.__class__.__name__, err
                )
