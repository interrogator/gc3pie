...
from gc3apps.usz.gsubbeast import GsubbeastApplication
...
def _get_valid_input(input_folder):
    ...
    return [
        ...
        if input_file.endswith(".txt") and mimetypes.guess_type(input_file)[0] == "text/plain"
    ]
...
class GtopasScript(SessionBasedScript):
    ...
    def __init__(self):
        SessionBasedScript.__init__(
            self, version=__version__, application=GtopasApplication, stats_only_for=GtopasApplication
        )

    def setup_args(self):
        self.add_param(
            "input_folder", type=existing_directory, help="Path to input folder containing valid input .txt files."
        )
    ...
    def new_tasks(self, extra):
        ...
        for input_file in _get_valid_input(self.params.input_folder):
            ...
            tasks.append(GtopasApplication(input_file, **extra_args))
