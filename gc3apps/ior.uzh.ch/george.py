...
# todo: to make this work, i had to indent everything below this if. surely that isn't right...
if __name__ == "__main__":
    ...
    class ValueFunctionIteration(SequentialTaskCollection):
        ...
        def __init__(
            self, executable, initial_values_file, total_iterations, slice_size=0, output_dir=TMPDIR, **extra_args
        ):
            """
            ...
            """
            assert slice_size >= 0, (
                "Argument `slice_size` to ValueFunctionIteration.__init__" " must be a non-negative integer."
            )
            ...
            self.jobname = extra_args.get("jobname", gc3libs.utils.basename_sans(initial_values_file))

            # create initial task and register it
            initial_task = ValueFunctionIterationPass(
                executable,
                initial_values_file,
                0,
                total_iterations,
        ...
        def next(self, iteration):
            ...
            if iteration == self.total_iterations:
                ...
                last_values_filename = os.path.join(self.datadir, self.values_filename_fmt % iteration)
        ...
        def terminated(self):
            ...
            gc3libs.log.debug(
                "SequentialTaskCollection %s done,"
                " now processing results into file '%s'..." % (self, output_filename)
            )
            output_file = open(output_filename, "w+")  # write+truncate
            output_csv = csv.writer(output_file)
            output_csv.writerow(["i"] + [("n=%d" % n) for n in range(1, len(self.tasks) + 1)])
            for i, values in enumerate(zip(*[task.output_values for task in self.tasks])):
    ...
    class ValueFunctionIterationPass(ParallelTaskCollection):
        ...
        def __init__(
            ...
            """
            assert slice_size >= 0, (
                "Argument `slice_size` to ValueFunctionIterationPass.__init__" " must be a non-negative integer."
            )
            assert isinstance(extra, dict), (
                "Argument `extra` to ValueFunctionIterationPass.__init__" " must be a dictionary instance."
            )
            ...
            self.jobname = "%s.%s" % ((parent or gc3libs.utils.basename_sans(input_values_file)), (fmt % iteration))
        ...
        def terminated(self):
            ...
            """
            gc3libs.log.debug("%s terminated, now collecting return values from sub-tasks ..." % self)
            self.output_values = []
            for task in self.tasks:
                if task.execution.exitcode != 0:
                    self.execution.exitcode = 1
                    gc3libs.log.error(
                        "%s: sub-task %s failed with exit code %d. Aborting." % (self, task, task.execution.exitcode)
                    )
                    return
    ...
    class ValueFunctionIterationApplication(Application):
        ...
        def __init__(
            ...
            **extra_args
            ...
            extra_args.setdefault(
                "jobname", "%s.%d--%d" % (parent or gc3libs.utils.basename_sans(input_values_file), start, end)
            )
            Application.__init__(
                ...
                arguments=[extra_args["discount_factor"], iteration, total_iterations, count, start + 1, end + 1],
                inputs={executable: os.path.basename(executable), input_values_file: IN_VALUES_FILE},
        ...
        def terminated(self):
            gc3libs.log.debug(
                "%s: TERMINATED with return code %d ..." % (self.persistent_id, self.execution.returncode)
            )
            if self.execution.returncode == 0:
                ...
                try:
                    ...
                    msg = "Invalid content in file '%s' at line %d: %s" % (output_filename, lineno, str(ex))
                    ...
                    msg = "I/O error processing output file '%s': %s" % (output_filename, str(ex))
                    gc3libs.log.error("%s: %s" % (self.persistent_id, msg))
                    self.info = msg
                    self.exitcode = 74  # EX_IOERR in /usr/include/sysexits.h
                except Exception as ex:
                    msg = "Error processing result file '%s': %s" % (output_filename, str(ex))
                    gc3libs.log.error("%s: %s" % (self.persistent_id, msg))
                    self.info = msg
    ...
    class GeorgeScript(SessionBasedScript):
        ...
        def setup_args(self):
            ...
            self.add_param(
                ...
                type=positive_int,
                default=1,
                help="Compute NUM iterations per each output file" " (default: %(default)s).",
            )
            self.add_param(
                "-p",
                "--slice-size",
            ...
            self.add_param(
                ...
                default="vfi.exe",
                metavar="EXECUTABLE",
                help="Run the specified EXECUTABLE program" " to compute one step of values;" " default: %(default)s.",
            )
        ...
        def new_tasks(self, extra):
            ...
            for filename in inis:
                if filename not in successfully_read:
                    self.log.error("Could not read/parse input file '%s'. Ignoring it." % filename)

            for name in p.sections():
                # path to the initial values file
                if not p.has_option(name, "initial_values_file"):
                    self.log.error(
                        "Required parameter 'initial_values_file' missing" " in input '%s'.  Ignoring." % name
                    )
                ...
                if not os.path.exists(path):
                    self.log.error(
                        "Input values file '%s' does not exist."
                        " Ignoring task '%s', which depends on it." % (path, name)
                    )
                ...
                try:
                    extra_args.setdefault("discount_factor", p.getfloat(name, "discount_factor"))
                except (KeyError, ConfigParser.Error, ValueError) as ex:
                    self.log.error(
                        "Could not read required parameter 'discount_factor'" " in input '%s': %s" % (name, str(ex))
                    )

                yield (
                    name,
                    george.ValueFunctionIteration,
                    [self.params.execute, path, self.params.iterations, self.params.slice_size],
                    extra_args,
                )
        ...
        def print_summary_table(self, output, stats):
            ...
            for toplevel in self.session:
                ...
                for state in [
                    ...
                    count = stats[state]
                    if count > 0:
                        progresses.append("%.1f%% %s" % (100.0 * count / (total_tasks - 1), state))
                        if state == "TERMINATED":
