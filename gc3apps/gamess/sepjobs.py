...
import cli.log
from gc3libs.cmdline import existing_directory, existing_file
...
def CLEANLIST(itmlst, delstr):
    delpos = []
    p = 0  # list of deleting position, position
    for itm in itmlst:
...
def search_for_input_directories(root, name):
    ...
    """
    name = name.strip("/")
    print("DEBUG: stripped name of input folder: {}".format(name))
    result = []
...
def parse_kwfile(filename, old_search):
    ...
    with open(filename, "r") as configfile:
        for line in configfile:
            ...
            print(
                "DEBUG: search string, searchtype, foldername, start, end:\n",
                '"',
                what_to_search,
                '"',
                searchtype,
            ...
            if searchtype in ("regexp", "re"):
                result.append([foldername, re.compile(what_to_search), start, end, folder_exists])
...
def sepjobs(cmdline):
    ...
    dirs = search_for_input_directories(cmdline.params.search_root, cmdline.params.inpdir)
    wrkdir = dirs[0]
    ...
    def act_on_file(filepath, foldername, assigned, folder_exists, old_search):
        ...
        if cmdline.params.move:
            ...
            if dirpath0 != commonpath:
                dirpath1 = dirpath0.replace(commonpath, os.path.join(wrkdir, foldername), 1)
                try:
                ...
                os.rename(filepath, os.path.join(wrkdir, foldername, os.path.basename(filepath)))
    ...
    for filepath in files:
        ...
        for i in range(Nsrchkeys):
            ...
            if match:
                ...
                hitrt += 1
                assigned, srchkey[i][4] = act_on_file(filepath, srchkey[i][0], True, srchkey[i][4], old_search)
                break
        ...
        if not assigned:
            ...
            for i in range(Nsrchkeys):
                ...
                if match:
                    ...
                    break
        if not assigned:
            assigned, none_found_exists = act_on_file(filepath, "none_found", False, none_found_exists, old_search)
        inputfile.close()
...
sepjobs.add_param(
    "kwfile", type=existing_file, help="Path to the file containing foldername to string search mappings."
)
sepjobs.add_param(
    "inpdir",
    type=existing_directory,  # no support of several input dir.s
...
sepjobs.add_param(
    ...
    metavar="EXT",
    default=".out",
    help="Restrict search to file with this extension." " (default: %(default)s)",
)
