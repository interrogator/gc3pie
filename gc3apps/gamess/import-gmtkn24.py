...
class HtmlTable(object):
    ...
    def rows_as_dict(self):
        ...
        for row in self._rows:
            yield dict([(th[0], row[n]) for n, th in enumerate(self._headers)])
    ...
    def extract_html_table(html):
        ...
        rows = [list(grouped(row, spans)) for row in HtmlTable.extract_html_table_rows(table)]
        # all done
        return (headers, rows)
    ...
    def extract_html_table_rows(table):
        ...
        """
        return [
            tuple([str.join("", td.findAll(text=True)).strip() for td in tr.findAll("td")])
...
def turbomol_to_gamess(input_file_name, output_file_name, template_file_name, **extra_args):
    ...
    data = str.join(
        "\n", [("%-10s %3.1f  %+2.8f  %+2.8f  %+2.8f" % (a.symbol, a.nuclear_charge, a.x, a.y, a.z)) for a in atoms]
    )
...
cmdline.add_option(
    ...
    help="Import data into the specified directory." " Default: %default",
)
cmdline.add_option(
    "-t",
    "--template",
        ...
        if "import" == args[0]:
            ...
            for subset in subsets:
                ...
                if os.path.exists(readme):
                    charge_or_unpaired_re = re.compile(
                        r"(?P<molecule>[a-z0-9+_,-]+) \s+" r"\((?P<charge_or_unpaired>[+-]?[0-9]+)\)", re.I | re.X
                    )
                    ...
                    for line in readme_file:
                        ...
                        if match:
                            ...
                            if molecule not in molecules:
                                logger.warning(
                                    "Ignoring molecule '%s': mentioned by README file,"
                    ...
                    logger.info(
                        "No README file accompanies geometries; will use default: SCFTYP=RHF, ICHARG=0 and MULT=1"
                    )
                for molecule in molecules:
                    destfile = os.path.join(destdir, subset, molecule + ".inp")
                    logger.info(
                        "Generating GAMESS input file '%s' from geometry '%s' and template '%s'...",
                        destfile,
                        molecule,
                        template,
                    )
                    turbomol_to_gamess(tempdir + "/" + molecule, destfile, template, **params[molecule])
                ...
                output.write("#;" + "Systems" + (";" * maxwidth) + "Stoichiometry" + (";" * maxwidth) + "Ref.\n")
                for seqno, data in enumerate(refdata):
                    reaction, energy = data
                    width = len(reaction)
                    systems = str.join(";", reaction.keys() + ([" "] * (maxwidth - width)))
                    stoichiometry = str.join(
                        ";", [str(coeff) for coeff in reaction.values()] + ([" "] * (maxwidth - width))
                    )
...
"""
        ...
        logger.critical("HTTP error %d requesting page: %s" % (ex.code, ex.msg))
        sys.exit(1)
